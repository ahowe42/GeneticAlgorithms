...
def RunGASubset(params, data, objective, seedSubs=[], verbose=False, randSeed=None):
    ...
    bestAnn = dict(x=gens-1, y=bestScore, xref='x1', yref='y1', text='%s (%d) = %0.4f (%0.4f)'%\
        (BinaryStr(bestSubset), sum(bestSubset), bestScore, bestScore/fullScore),
    ...
    print('%s\nGA Complete\n\tTotal Nontrivial Solutions Possible - %d\n\tUnique Subsets Evaluated - %d (%0.2f%%)\n\tFull Subset Score = %0.4f\nTop %d Solutions'\
        %(dispLine, 2**p-1, len(allScores), 100*len(allScores)/(2**p-1), fullScore, min(showTopSubs, len(GA_BEST))))
